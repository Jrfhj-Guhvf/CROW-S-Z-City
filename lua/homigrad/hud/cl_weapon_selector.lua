...
local AcsentColor = Color(0,162,255)
...
function WS.WeaponSelectorDraw( ply )
    ...
    for i = 0, #Weapons do
        ...
        for Id = 0, #slotTbl do
            ...
            surface.SetDrawColor( 0, 162, 255, WS.Transparent*( SelectedWep == wep and 200 or 0 )  )
            surface.SetMaterial( gradient_u )
            surface.DrawTexturedRect( position, (scrH * 0.025) * (Ammout) + (scrH * 0.05) + lastPos, sizeX, sizeH )
            if SelectedWep == wep then
                surface.SetDrawColor( 0, 162, 255, WS.Transparent*155 )
